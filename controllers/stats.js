...
import urserStore from "../models/user-store.js";
const stats = {
    createView(request, response) {
        ...
        if (loggedInUser) {
            ...
            let userNum = urserStore.getAllUsers().length;

            let lastAddedPlaylist = playlists.reduce((latest, playlist) => {
                return new Date(playlist.created) > new Date(latest.created) ? playlist : latest;
            })
            const statistics = {
                ...
                userNum: userNum,
                lastAddedPlaylist: lastAddedPlaylist.title
            };
